        ...
        return (
            <React.Fragment key={blockIndex}>
                ...
                <section className="bottom-grid-section" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px', marginBottom: '50px', textAlign: 'left' }}>
                    {grid.map((news, i) => (
                        <div key={`${news.id}-${i}`} className="grid-item-container" style={{ paddingBottom: '20px' }}>
                            {/* Main Grid Item */}
                            <div className="grid-item" onClick={() => navigate(`/article/${news.id}`)} style={{ cursor: 'pointer', display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '10px' }}>
                                ...
                                <div className="grid-info">
                                    ...
                                    <p style={{ fontSize: '14px', color: '#666', margin: 0, lineHeight: '1.4', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', wordBreak: 'break-word' }}>
                                        {news.content}
                                    </p>
                                </div>
                            </div>
                ...
                {list.length > 0 && (
                    <>
                        ...
                        <section className="bottom-list-section" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', columnGap: '20px', rowGap: '30px', textAlign: 'left' }}>
                            {list.map((news, i) => (
                                <div key={`${news.id}-${i}`} className="list-item" onClick={() => navigate(`/article/${news.id}`)} style={{ cursor: 'pointer', display: 'flex', gap: '20px', alignItems: 'flex-start' }}>
