...
def add_news_reaction(
    ...
    # 뉴스 존재 확인
    news = db.query(AiGeneratedNews).filter(AiGeneratedNews.id == news_id).first()
    if not news:
    ...
    try:
        # 기존 반응 찾기
        reaction = (
            db.query(NewsReaction).filter(NewsReaction.user_id == user.id, NewsReaction.news_id == news_id).first()
        )
        ...
        if reaction:
            ...
            # 새 반응 추가
            reaction = NewsReaction(user_id=user.id, news_id=news_id, value=value)
            db.add(reaction)
            status_msg = "added"
...
def record_news_view(
    ...
    # 조회 기록 추가
    view = NewsView(user_id=user.id, news_id=news_id)
    db.add(view)
    db.commit()
